 ...
 * Return: nothing (void)
 */
int Map::getStartingRow() const {
    return startingRow_;
}

int Map::getStartingCol() const {
    return startingCol_;
}
void Map::resetMap()
{
...
char Map::getSpaceType(int row, int col) {
    if (isOnMap(row, col)) {
        if (isNPCLocation(row, col)) {
            return NPC;
        } else if (isRoomLocation(row, col)) {
            return ROOM;
        } else if (isDungeonExit(row, col)) {
            return EXIT;
        } else if (row == player_position_[0] && col == player_position_[1]) {
            return PARTY;
        } else {
...
int Map::getClearedRooms() const {
    int clearedRooms = 0;
    for (int i = 0; i < room_count_; i++) {
        int row = room_positions_[i][0];
        int col = room_positions_[i][1];
        if (map_data_[row][col] == EXPLORED) {
            clearedRooms++;
        }
    }
...
/* void Map::investigateSpace()
{
    int row = getPlayerRow();
    int col = getPlayerCol();
    char space_type = getSpaceType(row, col);

    if (space_type != EXPLORED)
    {
        map_data_[row][col] = EXPLORED;

        int random_event = rand() % 100;

        if (random_event < 10) // 10% chance of finding the key
        {
            // The player finds the key
            // Add your code here to handle finding the key
        }
        else if (random_event < 30) // 20% chance of finding a piece of treasure
        {
            // The player finds a piece of treasure
            // Add your code here to handle finding treasure based on the number of cleared rooms
        }
        else if (random_event < 50) // 20% chance of encountering a random monster
        {
            // The player encounters a random monster
            // Add your code here to handle fighting the monster

            if (monster_defeated) // Replace this with the actual condition to check if the monster is defeated
            {
                int key_drop_chance = rand() % 100;
                if (key_drop_chance < 10) // 10% chance of the monster dropping a key
                {
                    // The monster drops a key
                    // Add your code here to handle getting the key
                }
            }
        }

        // After investigating the space, every party member's fullness level has a 50% chance to drop by one point
        // Add your code here to handle updating the fullness level of each party member
    }
    else
    {
        // Inform the player that the space cannot be investigated again
        std::cout << "This space has already been investigated. Please select a different option." << std::endl;
    }
} */
